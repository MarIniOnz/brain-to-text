...
def rearrange_speech_logits_pt(logits):
    ...
    logits = np.concatenate(
        (logits[:, :, 0:1], logits[:, :, -1:], logits[:, :, 1:-1]), axis=-1
    )
    return logits
...
def runSingleDecodingStep(x, input_layer, model, model_args, device):
    ...
    with torch.autocast(
        device_type="cuda", enabled=model_args["use_amp"], dtype=torch.bfloat16
    ):
        x = gauss_smooth(
            inputs=x,
            device=device,
            smooth_kernel_std=model_args["dataset"]["data_transforms"][
                "smooth_kernel_std"
            ],
            smooth_kernel_size=model_args["dataset"]["data_transforms"][
                "smooth_kernel_size"
            ],
            padding="valid",
        )
...
def reset_remote_language_model(
    ...
    while len(remote_lm_done_resetting) == 0:
        ...
        if len(remote_lm_done_resetting) == 0:
            print(
                f"Still waiting for remote lm reset from ts {remote_lm_done_resetting_lastEntrySeen}..."
            )
...
def update_remote_lm_params(
    ...
    while len(remote_lm_done_updating) == 0:
        ...
        if len(remote_lm_done_updating) == 0:
            print(
                f"Still waiting for remote lm to update parameters from ts {remote_lm_done_updating_lastEntrySeen}..."
            )
...
def send_logits_to_remote_lm(
    ...
    while len(remote_lm_output) == 0:
        ...
        if len(remote_lm_output) == 0:
            print(
                f"Still waiting for remote lm partial output from ts {remote_lm_output_partial_lastEntrySeen}..."
            )
...
def finalize_remote_lm(
    ...
    while len(remote_lm_output) == 0:
        ...
        if len(remote_lm_output) == 0:
            print(
                f"Still waiting for remote lm final output from ts {remote_lm_output_final_lastEntrySeen}..."
            )
    ...
    for entry_id, entry_data in remote_lm_output[0][1]:
        ...
        candidate_sentences = [
            str(c) for c in entry_data[b"scoring"].decode().split(";")[::5]
        ]
        candidate_acoustic_scores = [
            float(c) for c in entry_data[b"scoring"].decode().split(";")[1::5]
        ]
        candidate_ngram_scores = [
            float(c) for c in entry_data[b"scoring"].decode().split(";")[2::5]
        ]
        candidate_llm_scores = [
            float(c) for c in entry_data[b"scoring"].decode().split(";")[3::5]
        ]
        candidate_total_scores = [
            float(c) for c in entry_data[b"scoring"].decode().split(";")[4::5]
        ]
