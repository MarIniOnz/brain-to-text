...
from model_training.dataset import BrainToTextDataset, train_test_split_indices
...
class BrainToTextDecoder_Trainer:
    ...
    def __init__(self, args):
        ...
        if args["mode"] == "train":
            ...
            fh = logging.FileHandler(
                str(pathlib.Path(self.args["output_dir"], "training_log.log"))
            )
        ...
        if torch.cuda.is_available():
            ...
            try:
                ...
                test_tensor = torch.tensor([1.0], device=self.device) * 2
            except Exception as e:
        ...
        if self.args["lr_scheduler_type"] == "linear":
            ...
            raise ValueError(
                f"Invalid learning rate scheduler type: {self.args['lr_scheduler_type']}"
            )
    ...
    def load_model_checkpoint(self, load_path):
        ...
        self.best_val_loss = (
            checkpoint["val_loss"] if "val_loss" in checkpoint.keys() else torch.inf
        )
    ...
    def create_attention_mask(self, sequence_lengths):
        ...
        key_mask = torch.arange(max_length, device=sequence_lengths.device).expand(
        ...
        attention_mask_float = torch.where(attention_mask, True, False)
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            # Clip gradient
            use_foreach = self.device.type == "cuda"
            ...
            if i % self.args["batches_per_val_step"] == 0 or i == (
                ...
                if new_best:
                    ...
                    if self.args["save_val_metrics"]:
                        with open(
                            f"{self.args['checkpoint_dir']}/val_metrics.pkl", "wb"
                        ) as f:
    ...
    def validation(
        self, loader: DataLoader, return_logits: bool = False, return_data: bool = False
    ):
        ...
        for _, batch in enumerate(loader):
            ...
            if return_logits:
                ...
                )  # Will be in bfloat16 if AMP is enabled, so need to set back to float32
